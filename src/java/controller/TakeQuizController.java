        ...
        if (request.getParameter("submitTakeQuiz") != null) {
            try {
                ...
                if (NumberQuestion.matches("[1-9]+")) {
                    ...
                    //make sure users enter the number of questions that are smaller
                    //than the number of existing questions
                    if (numberOfQuestion > totalQuest) {
                        request.setAttribute("errorTakeQuiz", "You must enter a number in rage [1," + totalQuest + "]");
                        request.setAttribute("body", "takeQuiz.jsp");
                    } else {
